...
import org.myrtle.atomic.IO.autoClose
...
open class ByteCache : org.myrtle.atomic.Bytes, AutoCloseable{
	...
	private fun createTempFile(): FileObject{
		val a = _root_ide_package_.org.myrtle.atomic.VirtualFileSystem.getRam("${this::class.java.typeName}/${UUID.randomUUID().toString().uppercase()}.${_root_ide_package_.org.myrtle.atomic.SystemConfig.of.getString("org.myrtle.canary.ByteCache.createTempFile.suffixName",".bin")}")
	...
	override fun getHexs(): List<String> {
		...
		for (b in getBytes()) {
			val hex = String.format(_root_ide_package_.org.myrtle.atomic.SystemConfig.of.getString("org.myrtle.canary.ByteCache.getHexs.hexFormat","%02X"), b.toInt() and 0xFF)
			hexList.add(hex)
		}
		return hexList
	}
