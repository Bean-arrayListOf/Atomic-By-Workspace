...
import org.myrtle.atomic.IO.IStreamMode.Local
import org.myrtle.atomic.IO.IStreamMode.NetWork
...
object IO {
	...
	@JvmStatic
	val getInputStreamArgs: Map<String, IStreamMode>

	enum class IStreamMode{
		ClassLoader,Local,NetWork
	}

	init {
	    val hmap = HashMap<String, IStreamMode>()

		hmap["cp:"] = IStreamMode.ClassLoader
		hmap["classpath:"] = IStreamMode.ClassLoader
		hmap["class-path:"] = IStreamMode.ClassLoader
		hmap["cl:"] = IStreamMode.ClassLoader
		hmap["classloader:"] = IStreamMode.ClassLoader
		hmap["class-loader:"] = IStreamMode.ClassLoader
		hmap["jar:"] = IStreamMode.ClassLoader
		hmap["f:"] = IStreamMode.Local
		hmap["file:"] = IStreamMode.Local
		hmap["local:"] = IStreamMode.Local
		hmap["http:"] = IStreamMode.NetWork
		hmap["https:"] = IStreamMode.NetWork

		this.getInputStreamArgs = hmap
	}
...
//	}
	...
	fun openStream(url: String): InputStream {
		...
		val ifUrl = url.lowercase()

		for (i in getInputStreamArgs){
			if (!ifUrl.startsWith(i.key)){
				continue
			}

			return when(i.value){
				IStreamMode.ClassLoader -> {
					val resourcePath = url.substringAfter(":").removePrefix("/")
					openResourceStream(resourcePath)
				}
				Local -> {
	...
	@JvmStatic
	fun openResourceStream(url: String): InputStream {
		val cl = Thread.currentThread().contextClassLoader
		return cl.getResource(url)?.openStream() ?: throw IOException("Resource $url not found")
	}
	...
	@JvmStatic
	fun openFile(url: String): InputStream {
		val file = File(url)
		if (!file.exists()) {
			throw FileNotFoundException("File not found: $url")
		}
	...
	@JvmStatic
	@Suppress("NOTHING_TO_INLINE")
	inline fun neoFile(file: String): File{
		return File(file)
	}
