...
class Mas {
	companion object{
		...
		fun main(vararg args: String) {
			val source = "/Users/cat/Downloads/Downie"
			val target = "/Users/cat/Downloads/Downie/1"
			val sourceDir = File(source)
			val targetDir = File(target)

			val delete = false

			val pattern = Pattern.compile(".*\\.ts", Pattern.CASE_INSENSITIVE)
			val initCode =
			...
			val files = IO.findFiles(sourceDir,option)
			val errors = ArrayList<String>()

			for (i in files.indices){
				val sourceName = files[i].name
				val targetName = sourceName.replace(".ts", ".mov")
