...
class BaseCore {
	 ...
	 *
	 * @param citrusAscii 编码字符串，格式为"字符表标识符:编码字符串"。
	 * @return 解码后的字节数组。
	 */
	fun dec(citrusAscii: String): ByteArray {
		// 将核心ASCII字符串按":"分割，以获取字符表标识符和编码字符串
		val temp = citrusAscii.split("$:")
	...
	fun dec(charTable: CharTable, citrusAscii: String): ByteArray {
		var str = citrusAscii
		for (c in charTable.replaceChars) {
